...
def make_os_graph(sim):
    # env_to_graph returns a 7-tuple on this branch (trailing wl_colours/wl_histogram,
    # added by cell4-wl-vilg's WL wiring, not present when this file was written on
    # cell5-atom-gnn) -- to_data only wants the original 5 graph-tensor fields.
    return to_data(*env_to_graph(sim)[:5])
