...
Ported from cell5-atom-gnn onto the cell4-wl-vilg base: TaxiWorldSimulator here HAS the
stale-edge fix (attempt_move/attempt_pickup remove both directions of a tether edge, not
just the forward one), unlike cell5-atom-gnn's own base (cell2-vilg-gnn). The atom
encoding is unaffected by stale reverse edges by construction either way: env_to_atoms
only ever reads attr[-1] == 1 (forward) edges, exactly like env_to_vilg_graph, so a stale
reverse edge (always attr[-1] == -1) can never produce a spurious atom -- this is checked
explicitly below (TestGroundTruth.test_after_several_moves_stale_reverse_edges_produce_no_atoms),
by injecting one by hand rather than relying on the now-fixed bug to produce it.
...
class TestGroundTruth(unittest.TestCase):
    ...
    def test_after_several_moves_stale_reverse_edges_produce_no_atoms(self):
        """Tests env_to_atoms' forward-edge-only filter (attr[-1] == 1) directly, on a
        FIXED simulator (this branch carries cell4-wl-vilg's stale-edge fix -- see
        taxi_world.py's attempt_move/attempt_pickup -- so the bug this test used to rely
        on to produce a stale edge no longer fires by itself). Instead, after several
        moves and a pickup, a stale reverse tether edge is injected by hand, in exactly
        the shape the old bug used to leave behind (attr[-1] == -1, location -> taxi, at
        a location the taxi is no longer at): env_to_atoms must be completely unaffected,
        since it only ever reads attr[-1] == 1 (forward) edges, exactly like
        env_to_vilg_graph."""
        for seed in CITY_SEEDS:
            with self.subTest(seed=seed):
                sim = make_city_sim(seed)
                ...
                # a location the taxi is no longer at, with no existing edge to/from node
                # 0 -- exactly where the old bug would have left a stale reverse edge
                stale_location = next(
                    loc for loc in range(1, len(sim.graph.nodes))
                    if loc != sim.taxi.location
                    and sim.graph.nodes[loc]["attr"] == [1, 0, 0]
                    and not sim.graph.has_edge(loc, 0)
                    and not sim.graph.has_edge(0, loc)
                )
                sim.graph.add_edge(stale_location, 0, attr=[0, 1, 0, -1])
                self.assertTrue(sim.graph.has_edge(stale_location, 0))

                self.assertEqual(env_to_atoms(sim), atoms_before)
