...
# pair at which they share it. This is the atom-encoding family (env_to_atoms /
# atoms_to_graph / graph_to_atoms / env_to_atom_graph / atoms_to_json / json_to_atom_graph
# / env_to_atom_json), wired into taxi_world / taxi_env / planner / the CLI via
# graph_convention="atom" -- env_to_graph and env_to_vilg_graph above are untouched.
...
def env_to_atoms(env):
    ...
    Atom order (and therefore atom/graph-row index) is:
      ...
      2. One atom per env.graph edge with attr[-1] == 1 -- the SAME forward-direction-only
         filter env_to_vilg_graph uses, so even a stale reverse edge left behind by a
         one-directional remove_edge in attempt_move/attempt_pickup (the bug cell4's
         taxi_world.py fixes -- irrelevant to this filter either way, since it only ever
         admits attr[-1] == 1 edges) can never produce a spurious atom: reverse edges
         always carry attr[-1] == -1, so they're filtered out here exactly as they always
         were for vILG. These are sorted by (u, v) -- NOT raw
